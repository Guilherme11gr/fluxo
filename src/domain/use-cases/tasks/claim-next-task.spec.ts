...
describe("buildDeterministicBranchName", () => {
  ...
  it("is deterministic for the same inputs", () => {
    const name1 = buildDeterministicBranchName(
      "abc12345-xxxx",
      "TASK",
      "builder",
      null,
    );
    const name2 = buildDeterministicBranchName(
      "abc12345-xxxx",
      "TASK",
      "builder",
      null,
  ...
  it("builds a semantic task branch when readable task metadata is available", () => {
    const name = buildDeterministicBranchName(
      ...
      "agent/",
      {
        localId: 387,
        projectKey: "FLXO",
        title: "Runner worktree fix validation",
      },
    );
    expect(name).toBe(
      "agent/codex-updated-runner-smoke/task-flxo-387-runner-worktree-fix-validation",
    );
  });
  ...
  it('defaults to "agent" when agent name results in empty slug', () => {
    const name = buildDeterministicBranchName("shortid", "TASK", "@@@", null);
...
describe("buildMemorySearchQuery", () => {
  it("joins title and description into a compact search query", () => {
    ...
    expect(query).toBe(
      "Deploy app na VPS Usar docker compose e validar nginx antes de reiniciar.",
    );
  });

  it("returns only the title when description is empty", () => {
    expect(buildMemorySearchQuery("Planejar memory v1", null)).toBe(
      "Planejar memory v1",
    );
  });
});
